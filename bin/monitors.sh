...
if [[ $(xrandr -q | grep 'DP-2 connected') ]]; then
  ...
  if xrandr | grep -q "2560x1080"; then
    ...
    xrandr --output DP-1 --primary --mode 1920x1080 --pos 1920x0 --rotate normal --output HDMI-1 --off --output DP-2 --off HDMI-2 --off --output DP-3 --off --output HDMI-3 --off &
  fi
fi
